...
export const get_status_from_code = (code: ErrorCode) => ErrorCodeMap.get(code);

export const get_code_from_status = (status: number) =>
  HttpStatusMap.get(status)?.[0] || ErrorCode.Unimplemented;
...
export const createErrorResponse = (err: unknown) => {
  if (err instanceof RequestError) {
    return { code: err.code, payload: err.message };
  }

  if (err instanceof DOMException) {
    const error =
      "TimeoutError" === err.name
        ? RequestError.deadlineExceeded(err.message)
        : "AbortError" === err.name
          ? RequestError.canceled(err.message)
          : "TypeError" === err.name
            ? RequestError.failedPrecondition(err.message)
            : RequestError.unknown(err.message);
